...
class usersDb {
public:
    virtual Retval verify_credentials(std::string username, std::string password) = 0;
    ...
    virtual Retval is_anonymous_questions_allowed(int user_id, bool &is_allowed) = 0;
...
public:
  usersDbCsv();
  Retval verify_credentials(std::string username, std::string password) override ;
